...
Page({
  ...
  onLoad: function() {
    ...
    /* 使用first_food_type_id从后端获取对应的food
    信息和所有的food_type的信息 */
    ...
    /* 处理food_types信息，使之包括“相应type的食物是否加载
    到前端”的信息 */
    var tmp_food_types = warpFoodTypesInfo(food_types);

    // console.log(tmp_food_types);
    // console.log(tmp_food);
  ...
  tabClick: function(e) {
    ...
    /* 向后端发送e.currentTarget.dataset.id，对应
    food_type_id，获取包含所有相应种类的食物的数组 */
    ...
    /* 改变相应food_type，表明前端已经拥有该type对应的食物
    的数据 */
    var tmp_food_types = this.data.food_types;
    tmp_food_types[e.currentTarget.dataset.id].loaded = true;

    /* 注意，此处的id是food_type_id，这样的用法意味着后端
    传输的foot_type数组必须按id大小排序，而且id间隔为1 */
    this.setData({
      food: tmp_food,
      food_types: tmp_food_types,
      active_type_index: e.currentTarget.dataset.id
